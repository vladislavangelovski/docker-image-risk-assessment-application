...
public final class ScanValidators {
    ...
    public static Summary computeSummary(List<Finding> findings) {
        ...
        for (Severity s : Severity.values()) by.put(s, 0);
        int fix = 0;
        for (Finding f : findings) {
            Severity s = f.severity() != null ? f.severity() : Severity.UNKNOWN;
            by.put(s, by.get(s) + 1);
            if (f.fixedVersion() != null && !f.fixedVersion().isBlank()) fix++;
        }
        return new Summary(findings.size(), by, fix);
    }

    public static boolean matches(Summary a, Summary b) {
        if (a == null || b == null) return false;
        if (a.total() != b.total()) return false;
        if (a.fixAvailable() != b.fixAvailable()) return false;
