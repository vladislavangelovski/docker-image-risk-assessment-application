...
public class GlobalExceptionHandler {
    private static ResponseEntity<ErrorResponse> build(HttpStatus status, ErrorCode code, String message, Map<String, Object> details) {
    ...
    public ResponseEntity<ErrorResponse> handleUnreadable(HttpMessageNotReadableException ex) {
        return build(HttpStatus.BAD_REQUEST, ErrorCode.BAD_REQUEST_IMAGE, "Malformed JSON request", Map.of());
    }
    ...
    public ResponseEntity<ErrorResponse> handleParser(ParserException ex) {
        return build(HttpStatus.UNPROCESSABLE_ENTITY, ErrorCode.TRIVY_PARSE_ERROR, ex.getMessage(), Map.of());
    }
    ...
    public ResponseEntity<ErrorResponse> handleCache(ScanCache.CacheWriteException ex) {
        return build(HttpStatus.INTERNAL_SERVER_ERROR, ErrorCode.CACHE_WRITE_ERROR, "Failed to cache scan result", Map.of());
    }
    ...
    public ResponseEntity<ErrorResponse> handleAny(Exception ex) {
        return build(HttpStatus.INTERNAL_SERVER_ERROR, ErrorCode.INTERNAL, "Unexpected server error", Map.of());
    }
}
