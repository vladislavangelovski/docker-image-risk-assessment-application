...
public class DefaultScanJobCoordinator implements ScanJobCoordinator {
    ...
    public DefaultScanJobCoordinator(ScanOrchestrator orchestrator,
                                     ScanJobStore store,
    ...
    public ScanJobStatus submit(ScanRequest request) throws ScanJobStore.StoreWriteException {
        ...
        ScanJobStatus queued = new ScanJobStatus(scanId, request.image(), ScanJobStatus.Status.QUEUED, null, createdAt,
                                                 null, null);
    ...
    private void run(UUID scanId,
                     ScanRequest request,
                     Instant createdAt) {
        Duration ttl = Duration.ofSeconds(properties.getJob().getTtlSeconds());
        Instant startedAt = Instant.now();
        updateStatus(new ScanJobStatus(scanId, request.image(), ScanJobStatus.Status.RUNNING, null, createdAt,
                                       startedAt, null), ttl);
        try {
            ...
            updateStatus(new ScanJobStatus(scanId, result.image(), ScanJobStatus.Status.SUCCEEDED, null, createdAt,
                                           startedAt, finishedAt), ttl);
        } catch (Exception ex) {
            Instant finishedAt = Instant.now();
            log.warn("[scan-service] Async scan failed for scanId={}: {}", scanId, ex.getMessage());
            updateStatus(new ScanJobStatus(scanId, request.image(), ScanJobStatus.Status.FAILED, "Scan failed",
                                           createdAt, startedAt, finishedAt), ttl);
        }
    }

    private void updateStatus(ScanJobStatus status,
                              Duration ttl) {
        try {
            store.put(status, ttl);
        } catch (ScanJobStore.StoreWriteException ex) {
            log.warn("[scan-service] Failed to persist scan job status for scanId={}", status.scanId(), ex);
        }
    }
}
