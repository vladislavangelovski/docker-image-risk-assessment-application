...
public class JacksonTrivyParser implements TrivyParser {
    ...
    public ParsedScan parse(String rawJson) throws ParserException {
            ...
            for (JsonNode result : results) {
                ...
                if (vulns != null && vulns.isArray()) {
                    for (JsonNode v : vulns) {
                        ...
                        findings.add(new Finding(vulnId, pkg, installed, emptyToNull(fixed),
                                                 severity != null ? severity : Severity.UNKNOWN,
                                                 emptyToNull(severitySource), cvss, List.copyOf(refs),
                                                 emptyToNull(target)));
        ...
        return new ParsedScan(imageName, digest, findings, bySeverity, fixAvailable);
    }
    
    
    private Finding toFinding(JsonNode vuln,
                              String target) {
        ...
        return new Finding(vulnId, pkg, installed, emptyToNull(fixed), severity != null ? severity : Severity.UNKNOWN,
                           emptyToNull(severitySource), cvss, List.copyOf(refs), emptyToNull(target));
    }
    ...
    private Cvss extractCvss(JsonNode cvssRoot) {
        ...
        //Pick highest score
        return candidates.stream().sorted((a, b) -> compareScore(b.score(), a.score())).findFirst().orElse(null);
    }
    
    private static int compareScore(BigDecimal a,
                                    BigDecimal b) {
        if (a == null && b == null) {
            return 0;
        }
    ...
    private Cvss readCvssEntry(String sourceName,
                               JsonNode node) {
        ...
        return new Cvss(sourceName != null ? sourceName.toLowerCase(Locale.ROOT) : "vendor", score, vector);
    }
