...
public class JdbcScanPersistence implements ScanPersistence {
  ...
  public void save(UUID scanId, ScanResult normalized, String rawJson) {
    ...
    MapSqlParameterSource scanParams =
    ...
    MapSqlParameterSource rawParams =
        new MapSqlParameterSource()
            .addValue("scan_id", scanId)
  ...
  private Optional<LoadedScan> loadSingle(
      String selectSql, MapSqlParameterSource params, boolean includeRaw) {
    var scans =
        jdbc.query(
            selectSql,
            params,
            (rs, rn) -> {
              ...
              var summary =
                  new com.finki.vladislavangelovski.scan_service.api.dto.Summary(
                      rs.getInt("total"), java.util.Map.copyOf(sev), rs.getInt("fix_available"));
    ...
    var findings =
        jdbc.query(
            SQL_SELECT_FINDINGS,
            new MapSqlParameterSource("scan_id", scanId),
            (rs, rn) -> {
              var cvss =
                  rs.getString("cvss_source") != null
                          || rs.getBigDecimal("cvss_score") != null
                          || rs.getString("cvss_vector") != null
    ...
    var scanResult =
        new com.finki.vladislavangelovski.scan_service.api.dto.ScanResult(
            scanId,
            (String) row[1],
            (String) row[2],
            (String) row[3],
  ...
  private MapSqlParameterSource mapFinding(
      UUID scanId, com.finki.vladislavangelovski.scan_service.api.dto.Finding f) {
    ...
    try {
      refsJson =
          mapper.writeValueAsString(
              f.references() == null ? java.util.List.of() : f.references()
