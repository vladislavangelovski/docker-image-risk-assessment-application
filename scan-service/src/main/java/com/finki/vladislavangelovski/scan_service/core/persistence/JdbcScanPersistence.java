...
public class JdbcScanPersistence implements ScanPersistence {
    ...
    public JdbcScanPersistence(NamedParameterJdbcTemplate jdbc,
                               ObjectMapper mapper) {
        this.jdbc = jdbc;
        this.mapper = mapper;
    }
    ...
    private static int sevCount(Map<Severity, Integer> map,
                                Severity key) {
    ...
    public void save(UUID scanId,
        ...
        MapSqlParameterSource scanParams = new MapSqlParameterSource().addValue("scan_id", scanId)
                ...
                .addValue("finished_at", Timestamp.from(normalized.finishedAt()))
        ...
        MapSqlParameterSource rawParams = new MapSqlParameterSource().addValue("scan_id", scanId)
    ...
    private Optional<LoadedScan> loadSingle(String selectSql,
        ...
        var findings = jdbc.query(SQL_SELECT_FINDINGS, new MapSqlParameterSource("scan_id", scanId), (rs, rn) -> {
            ...
            try {
                ...
                refs = json == null ? java.util.List.of() : mapper.readValue(json,
            ...
            return new com.finki.vladislavangelovski.scan_service.api.dto.Finding(rs.getString("cve_id"),
                                                                                  ...
                                                                                  rs.getString("fixed_version"), sev,
        ...
        String raw = null;
        if (includeRaw) {
            raw = jdbc.query(SQL_SELECT_RAW, new MapSqlParameterSource("scan_id", scanId),
                             (rs) -> rs.next() ? rs.getString(1) : null);
        }
        
        var scanResult = new com.finki.vladislavangelovski.scan_service.api.dto.ScanResult(scanId, (String) row[1],
                                                                                           (String) row[2],
                                                                                           (String) row[3],
    ...
    private MapSqlParameterSource mapFinding(UUID scanId,
        ...
        String refsJson;
        try {
            refsJson = mapper.writeValueAsString(f.references() == null ? java.util.List.of() : f.references()
                                                 // if getters: f.getReferences()
            );
        } catch (Exception e) {
            refsJson = "[]";
        }
        
        return new MapSqlParameterSource().addValue("scan_id", scanId)
