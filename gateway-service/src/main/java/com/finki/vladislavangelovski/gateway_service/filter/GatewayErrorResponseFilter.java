...
public class GatewayErrorResponseFilter implements GlobalFilter, Ordered {
            ...
            public Mono<Void> writeWith(Publisher<? extends DataBuffer> body) {
                ...
                return DataBufferUtils.join(body).flatMap(buffer -> {
                    ...
                    try {
                        ...
                        String fallback = "{\"status\":%d,\"message\":\"Upstream error\"}"
    ...
    private ErrorResponse buildError(ServerWebExchange exchange,
                                     HttpStatusCode status,
                                     byte[] body) {
        String requestId = exchange.getRequest().getHeaders().getFirst(RequestIdFilter.HEADER);
        String message = extractMessage(body);
        HttpStatus resolved = HttpStatus.resolve(status.value());
        if ((message == null || message.isBlank() || "Upstream error".equals(message)) && resolved != null) {
