...
public class CveAggregateController {
  ...
  public CveAggregateController(WebClient webClient, ServiceEndpointsProperties properties) {
    this.webClient = webClient;
    this.properties = properties;
  }

  @GetMapping("/api/v1/aggregate/cves/{id}")
  public Mono<CveAggregateResponse> getCveWithEpss(
    ...
    return Mono.zip(cve, epss).map(tuple -> new CveAggregateResponse(tuple.getT1(), tuple.getT2()));
  }

  private static void addRequestId(org.springframework.http.HttpHeaders headers, String requestId) {
    if (requestId != null && !requestId.isBlank()) {
      headers.set(RequestIdFilter.HEADER, requestId);
    }
  }

  public record CveAggregateResponse(CveEntryDto cve, List<EpssScoreDto> epssScores) {}
}
