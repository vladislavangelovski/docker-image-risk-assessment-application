    ...
    public Mono<GatewayHealth> checkDependencies() {
        ...
        return Mono.zip(scanHealth, cveStoreHealth, aiHealth)
                .map(tuple -> {
                    ...
                    boolean allHealthy = dependencies.values().stream().allMatch(DependencyHealth::isHealthy);
        ...
        return webClient
                ...
                .onErrorResume(
                        ex -> {
                            ...
                            return Mono.just(new DependencyHealth(name, HttpStatus.SERVICE_UNAVAILABLE.getReasonPhrase()));
                        });
    }
