...
public class SemanticClaimService {
  ...
  public QaClaimResponse judgeClaim(
    ...
    if (!mentionedGrounded.isEmpty() && looksLikePositivePresenceClaim(claim)) {
      ...
      rationaleBullets =
          List.of("The image scan detected " + cves + ".", "This directly supports the claim.");
    }
  ...
  public QaClaimResponse judgeClaim(
      QaClaimRequest request, Set<String> allowedCves, Map<String, List<String>> packagesByCve) {
  ...
  private static String formatRationale(
      Verdict verdict, List<String> bullets, Set<String> groundedCves) {
    List<String> effective = new ArrayList<>(bullets != null ? bullets : List.of());
    if (effective.isEmpty()) {
      switch (verdict) {
        case SUPPORTS ->
            effective.add("Evidence suggests the claim is supported by the retrieved CVE context.");
        case REFUTES ->
            effective.add(
                "Evidence suggests the claim is contradicted by the retrieved CVE context.");
        default ->
            effective.add(
                "Insufficient evidence in the retrieved CVE context to confirm or deny the claim.");
      }
    }
    ...
    for (ScanFinding f : selected) {
      ...
      String title =
          "Scan finding: "
              + cve
              + " ("
              + safeStr(f.packageName())
              + " "
              + safeStr(f.installedVersion())
              + ")"
              + epssSnippet;
      ...
      if (f.cvss() != null && f.cvss().score() != null) {
        sb.append(" (CVSS ")
            .append(String.format(java.util.Locale.ROOT, "%.1f", f.cvss().score()))
            .append(")");
      }
