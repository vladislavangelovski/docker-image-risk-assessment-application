...
public class EmbeddingIndexService {
  ...
  /**
   * Dev-friendly cursor used only by the “index next batch” endpoint to move past page 0 without
   * the caller having to supply a page param.
   *
   * <p>NOTE: This is intentionally in-memory (single instance in docker-compose).
   */
  private final AtomicInteger autoPageCursor = new AtomicInteger(0);

  public EmbeddingIndexService(
      CveStoreClient cveClient, EmbeddingsClient embeddings, VectorStoreRepository vectorRepo) {
        ...
        if (epss != null) {
          enriched.add(
              new CveForEmbedding(
                  ...
                  d.lastModified(),
                  d.references(),
                  toDouble(epss.getScore()),
                  toDouble(epss.getPercentile())));
        } else {
          enriched.add(d);
        }
      } catch (Exception ex) {
  ...
  public int indexNextBatch(int batchSize) {
    ...
    for (String id : ids) {
      ...
      enrichedDocs.add(
          e != null ? e : docs.stream().filter(x -> id.equals(x.cveId())).findFirst().orElse(null));
    }
