...
public class WebClientConfig {
  ...
  private ExchangeFilterFunction retryFilter() {
            ...
            .doBeforeRetry(
                rs -> {
                  long exponentialDelay =
                      (long) (retryInitialBackoffMs * Math.pow(2, rs.totalRetries()));
    ...
    return (request, next) ->
        next.exchange(request).flatMap(this::propagate5xxToError).retryWhen(retrySpec);
  }
  ...
  private ExchangeFilterFunction requestIdFilter() {
    return (request, next) -> {
      ...
      ClientRequest mutated = ClientRequest.from(request).header("X-Request-Id", requestId).build();
