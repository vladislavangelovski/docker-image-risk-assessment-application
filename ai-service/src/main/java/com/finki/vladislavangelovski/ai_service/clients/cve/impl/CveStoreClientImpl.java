...
public class CveStoreClientImpl implements CveStoreClient {
  ...
  /**
   * Central place that converts CVE + optional EPSS into CveForEmbedding. EPSS from EpssScoreDto
   * (latest) has priority; if null, we fall back to the flattened epssScore/epssPercentile on
   * CveEntryDto (if present).
   */
  ...
  public CveForEmbedding getById(String cveId) {
    var base =
        cveWebClient.get().uri(byIdPath, cveId).retrieve().bodyToMono(CveEntryDto.class).block();
