...
public class StartupEmbeddingIndexer {
  ...
  public StartupEmbeddingIndexer(
  ...
  public void runOnceImmediately() {
    ...
    taskExecutor.execute(
        () -> {
          ...
          log.info(
              "Startup embeddings indexing: batchSize={}, maxBatches={}",
              safeBatchSize,
              safeMaxBatches);
          ...
          for (int i = 0; i < safeMaxBatches; i++) {
            ...
            if (upserted < safeBatchSize) {
              log.info(
                  "Startup embeddings indexing: short batch ({} < {}), stopping",
                  upserted,
                  safeBatchSize);
              break;
            }
          ...
          log.info(
              "Startup embeddings indexing complete: upserted {} across {} batches",
              totalUpserted,
              batches);
        });
  }
